...
function parsePhatNguoiVnHtml(html) {
	...
	if (!html || /không có lỗi vi phạm/i.test(html)) {
		return empty;
	}
	...
	for (const table of tables) {
		...
		for (const row of rows) {
			...
			while ((m = cellRe.exec(row)) !== null) {
				cells.push(htmlCellToText(m[1]));
			}
			if (cells.length >= 2) {
				const label = cells[0].replace(/[:\s]+$/, "").trim();
				let value = cells[1].trim();
				if (label === "Hành vi vi phạm") {
					value = value.replace(/\s*\(Xem chi tiết\)\s*$/i, "").replace(/\s*Xem mức phạt[\s\S]*$/i, "").trim();
				}
				if (label) record[label] = value;
			}
		}
		if (record["Biển kiểm soát"] || record["Thời gian vi phạm"] || record["Hành vi vi phạm"]) {
			const noi = record["Nơi giải quyết vụ việc"];
			record["Nơi giải quyết vụ việc"] = noi
	...
	let daxuphat = 0;
	for (const v of data) {
		const tt = (v["Trạng thái"] || "").toLowerCase();
		if (tt.includes("đã xử")) daxuphat++;
		else chuaxuphat++;
	}
	...
	return {
		...
		data_info: {
			...
			chuaxuphat,
			daxuphat,
			latest: data[0]["Thời gian vi phạm"] || null,
		},
	};
}

// Goi 1 lan toi phatnguoi.vn voi 1 loai xe cu the
async function fetchPhatNguoiVnHtml(cleanPlate, loaiXe) {
	const controller = new AbortController();
	const timer = setTimeout(() => controller.abort(), 12000);
	try {
		const url = `${API_URL_FALLBACK}/${encodeURIComponent(cleanPlate)}/${loaiXe}`;
		...
		if (!res.ok) throw new Error(`phatnguoi.vn HTTP ${res.status}`);
...
// Nguon du phong: api.phatnguoi.vn (tra ve HTML). Thu oto (1) roi xe may (2).
async function fetchFromPhatNguoiVn(bienSo) {
	const clean = bienSo.replace(/[-.\s]/g, "").toUpperCase();
	const types = ["1", "2"];
	let lastClean = null;
	let lastErr = null;
	for (const loai of types) {
		try {
			const html = await fetchPhatNguoiVnHtml(clean, loai);
			const parsed = parsePhatNguoiVnHtml(html);
	...
	console.error("[PHATNGUOIVN_ERROR]", lastErr);
	throw lastErr || new Error("phatnguoi.vn khong phan hoi");
}

// Ham chinh: uu tien nguon chinh, that bai thi dung nguon du phong
async function fetchPhatNguoi(bienSo) {
	try {
		return await fetchFromCheckPhatNguoi(bienSo);
	} catch (primaryErr) {
		console.error("[API_PRIMARY_FAIL]", primaryErr && primaryErr.message);
		try {
			const result = await fetchFromPhatNguoiVn(bienSo);
			console.log("[API_FALLBACK_OK] phatnguoi.vn");
			return result;
		} catch (fallbackErr) {
			console.error("[API_FALLBACK_FAIL]", fallbackErr && fallbackErr.message);
...
3️⃣ TÍNH PHÁP LÝ:
    ...
    • Nguồn dữ liệu từ API công khai: api.checkphatnguoi.vn (chính) & api.phatnguoi.vn (dự phòng)
